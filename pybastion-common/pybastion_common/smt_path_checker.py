...
from pybastion_common.models import ExecutionItem
...
class PathFeasibilityChecker:
    ...
    def check_path(self, path_eis: list[ExecutionItem]) -> PathFeasibilityResult:
        """
        Check if a path through the CFG is feasible.

        Args:
            path_eis: List of ExecutionItem objects representing the path
        ...
        for ei in path_eis:
            # Only process condition-type eis
            if ei.constraint is None or ei.constraint.constraint_type != "condition":
                continue

            if not ei.constraint.expr:
                continue
            ...
            constraint = extractor.parse_condition_to_z3(
                ei.constraint.expr,
                (
                    ei.constraint.polarity
                    if ei.constraint.polarity is not None
                    else True
                ),
            )
...
def filter_feasible_paths(
    ...
    Args:
        ...
        execution_items: List of all ExecutionItem objects
        timeout_ms: Solver timeout in milliseconds
    ...
    # Create EI lookup
    ei_map = {b.id: b for b in execution_items}
    ...
    for idx, path in enumerate(all_paths):
        if verbose:
            ...
            print(
                f"\r  progress: {pct}% ({idx + 1}/{total_paths} paths)",
                end="",
                flush=True,
            )

        # Get EIs for this path
        path_eis = [ei_map[ei_id] for ei_id in path if ei_id in ei_map]

        # Check feasibility
        result = checker.check_path(path_eis)
