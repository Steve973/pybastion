...
class AssignDecomposer(SingularStatementDecomposer):
    @classmethod
    def statement_parts(
        cls, stmt: ast.Assign, context: DecompositionContext
    ) -> list[StatementPart]:
        return [StatementPart(stmt.value)]

    @classmethod
    def semantic_eis(
        cls, stmt: ast.Assign, source_lines: list[str], context: DecompositionContext
    ) -> list[DecomposerResult]:
        text = (
            source_lines[stmt.lineno - 1].strip()
            if stmt.lineno <= len(source_lines)
            else ast.unparse(stmt)
        )
...
class AugAssignDecomposer(SingularStatementDecomposer):
    @classmethod
    def statement_parts(
        cls, stmt: ast.AugAssign, context: DecompositionContext
    ) -> list[StatementPart]:
        return [StatementPart(stmt.value)]

    @classmethod
    def semantic_eis(
        cls, stmt: ast.AugAssign, source_lines: list[str], context: DecompositionContext
    ) -> list[DecomposerResult]:
        text = (
            source_lines[stmt.lineno - 1].strip()
            if stmt.lineno <= len(source_lines)
...
class AnnAssignDecomposer(SingularStatementDecomposer):
    @classmethod
    def statement_parts(
        cls, stmt: ast.AnnAssign, context: DecompositionContext
    ) -> list[StatementPart]:
        return [StatementPart(stmt.value)] if stmt.value else []

    @classmethod
    def semantic_eis(
        cls, stmt: ast.AnnAssign, source_lines: list[str], context: DecompositionContext
    ) -> list[DecomposerResult]:
        text = (
            source_lines[stmt.lineno - 1].strip()
            if stmt.lineno <= len(source_lines)
...
class ExprDecomposer(SingularStatementDecomposer):
    @classmethod
    def decompose(
        cls, stmt: ast.Expr, source_lines: list[str], context: DecompositionContext
    ) -> list[DecomposerResult]:
    ...
    @classmethod
    def statement_parts(
        cls, stmt: ast.Expr, context: DecompositionContext
    ) -> list[StatementPart]:
        return [StatementPart(stmt.value)]
...
class ReturnDecomposer(SingularStatementDecomposer):
    @classmethod
    def statement_parts(
        cls, stmt: ast.Return, context: DecompositionContext
    ) -> list[StatementPart]:
        return [StatementPart(stmt.value)] if stmt.value else []

    @classmethod
    def semantic_eis(
        cls, stmt: ast.Return, source_lines: list[str], context: DecompositionContext
    ) -> list[DecomposerResult]:
...
class RaiseDecomposer(SingularStatementDecomposer):
    @classmethod
    def statement_parts(
        cls, stmt: ast.Raise, context: DecompositionContext
    ) -> list[StatementPart]:
        return [StatementPart(stmt.exc)] if stmt.exc else []

    @classmethod
    def semantic_eis(
        cls, stmt: ast.Raise, source_lines: list[str], context: DecompositionContext
    ) -> list[DecomposerResult]:
...
class BreakDecomposer(SingularStatementDecomposer):
    @classmethod
    def semantic_eis(
        cls, stmt: ast.Break, source_lines: list[str], context: DecompositionContext
    ) -> list[DecomposerResult]:
        loop_owner = context.nearest_loop
        return [
            semantic(
                "break",
                loop_owner.next_stmt_lines if loop_owner else context.next_stmt_lines,
            )
        ]
...
class ContinueDecomposer(SingularStatementDecomposer):
    @classmethod
    def semantic_eis(
        cls, stmt: ast.Continue, source_lines: list[str], context: DecompositionContext
    ) -> list[DecomposerResult]:
        loop_owner = context.nearest_loop
        return [
            semantic(
...
class PassDecomposer(SingularStatementDecomposer):
    @classmethod
    def semantic_eis(
        cls, stmt: ast.Pass, source_lines: list[str], context: DecompositionContext
    ) -> list[DecomposerResult]:
...
class DeleteDecomposer(SingularStatementDecomposer):
    @classmethod
    def semantic_eis(
        cls, stmt: ast.Delete, source_lines: list[str], context: DecompositionContext
    ) -> list[DecomposerResult]:
        return [
            semantic(
                f"del {', '.join(ast.unparse(t) for t in stmt.targets)}",
...
class ImportDecomposer(SingularStatementDecomposer):
    @classmethod
    def semantic_eis(
        cls, stmt: ast.Import, source_lines: list[str], context: DecompositionContext
    ) -> list[DecomposerResult]:
        return [
            semantic(
                f"import {', '.join(alias.name for alias in stmt.names)}",
                context.next_stmt_lines,
            )
        ]
...
class AssertDecomposer(SingularStatementDecomposer):
    @classmethod
    def statement_parts(
        cls, stmt: ast.Assert, context: DecompositionContext
    ) -> list[StatementPart]:
    ...
    @classmethod
    def semantic_eis(
        cls, stmt: ast.Assert, source_lines: list[str], context: DecompositionContext
    ) -> list[DecomposerResult]:
...
class GlobalDecomposer(SingularStatementDecomposer):
    @classmethod
    def semantic_eis(
        cls, stmt: ast.Global, source_lines: list[str], context: DecompositionContext
    ) -> list[DecomposerResult]:
...
class NonlocalDecomposer(SingularStatementDecomposer):
    @classmethod
    def semantic_eis(
        cls, stmt: ast.Nonlocal, source_lines: list[str], context: DecompositionContext
    ) -> list[DecomposerResult]:
...
class DefaultDecomposer(SingularStatementDecomposer):
    @classmethod
    def semantic_eis(
        cls, stmt: ast.stmt, source_lines: list[str], context: DecompositionContext
    ) -> list[DecomposerResult]:
        text = (
            source_lines[stmt.lineno - 1].strip()
            if stmt.lineno <= len(source_lines)
